...
export default function InvestCard({
    ...
    const handlePayment = () => {
        setProducts(prevState => {
            ...
            if(productIndex === -1) return [...prevState, {name: name, sum: sum, type: "investments"}]
            return prevState.map((product, index) => {
                if(productIndex === index) {
                    return {...prevState[productIndex], sum: prevState[productIndex].sum + sum, type: "investments"}
                }
                return product;
            });
        })
